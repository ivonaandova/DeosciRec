...
class DGCF(object):
    def __init__(self, data_config, pretrain_data):
        ...
        self.model_type += '_%s_%s_l%d' % (
        self.adj_type, self.alg_type, self.n_layers)  # For ex. DGCF_osci_laplacian_no_eye_dgcf_l4
        ...
        *********************************************************
        ...
        self.pos_i_g_embeddings_pre = tf.nn.embedding_lookup(self.weights['item_embedding'],
                                                             self.pos_items)  # items that the user interacted with
        self.neg_i_g_embeddings_pre = tf.nn.embedding_lookup(self.weights['item_embedding'],
                                                             self.neg_items)  # items that the user hasn't interacted with
        ...
        *********************************************************
        ...
        self.mf_loss, self.emb_loss, self.reg_loss = self.create_bpr_loss(self.u_g_embeddings, self.pos_i_g_embeddings,
    ...
    def _init_weights(self):
        ...
        if self.pretrain_data is None:
            ...
            all_weights['user_embedding'] = tf.Variable(
                initial_value=self.pretrain_data['user_embed'][:, 0:self.emb_dim], trainable=True,
                name='user_embedding', dtype=tf.float32)
            all_weights['item_embedding'] = tf.Variable(
                initial_value=self.pretrain_data['item_embed'][:, 0:self.emb_dim], trainable=True,
                name='item_embedding', dtype=tf.float32)
            print('using pretrained embeddings')
        for k in range(self.n_layers):
            all_weights['locality_%d' % k] = tf.Variable(initializer([self.n_users + self.n_items, 1]),
                                                         name='locality_%d' % k)
        return all_weights
    ...
    def _create_dgcf_embed(self):
        ...
        if self.node_dropout_flag:  # goes here because of using the default value for node_dropout_flag which is 1
            ...
            del self.norm_adj, self.cross_adj
        ego_embeddings = tf.concat([self.weights['user_embedding'], self.weights['item_embedding']],
                                   axis=0)  # concatenates into one tensor by one dimension (rows)
        all_embeddings = ego_embeddings / (
                    self.n_layers + 1)  # after the L-layer propagation we average the embeddings at each layer to contrust the final embedding for prediction
        for k in range(0, self.n_layers):
            temp_embed = []
            ego_embeddings = tf.multiply(tf.nn.sigmoid(self.weights['locality_%d' % k]),
                                         ego_embeddings)  # sigmoid is from the LA layer that is before every CHP layer
            for f in range(self.n_fold):  # save memory
                temp_embed.append(
                    tf.sparse_tensor_dense_matmul(A_fold_hat[f], ego_embeddings))  # sum messages of neighbors.
    ...
    def create_bpr_loss(self, users, pos_items, neg_items):
        pos_scores = tf.reduce_sum(tf.multiply(users, pos_items), axis=1)  # reduces dimensionality by columns, returs a sum
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.save_flag == 1:  # I use the default which is 1
        weights_save_path = '%sweights/%s/%s/%s/l%s_r%s_d%s_low%s_high%s' % (
        args.weights_path, args.dataset, model.model_type, layer, str(args.lr),
        '-'.join([str(r) for r in eval(args.regs)]), str(eval(args.node_dropout)[0]), str(args.low), str(args.high))
    ...
    *********************************************************
    ...
    if args.pretrain == 1:
        pretrain_path = '%sweights/%s/%s/%s/l%s_r%s_d%s_low%s_high%s' % (
        args.weights_path, args.dataset, model.model_type, layer, str(args.lr),
        '-'.join([str(r) for r in eval(args.regs)]), str(eval(args.node_dropout)[0]), str(args.low), str(args.high))
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):  # for each epoch (500)
        ...
        for idx in range(n_batch):  # iterates 30 times per epoch (It will take 30 samples in epoch)
            ...
            _, batch_loss, batch_mf_loss, batch_emb_loss, batch_reg_loss = sess.run(
                ...
                feed_dict={model.users: users, model.pos_items: pos_items, model.node_dropout: eval(args.node_dropout),
                           model.mess_dropout: eval(args.mess_dropout), model.neg_items: neg_items})
        ...
        cur_best_pre_0, stopping_step, should_stop = early_stopping(ret['recall'][0], cur_best_pre_0, stopping_step,
                                                                    expected_order='acc', flag_step=args.stop_step)
        # *********************************************************
        ...
        if ret['recall'][0] == cur_best_pre_0 and args.save_flag == 1:  # default args.save_flag 1
            ...
            u_embed, i_embed = sess.run([model.u_g_embeddings, model.pos_i_g_embeddings],
                                        feed_dict={model.users: list(range(model.n_users)),
                                                   model.pos_items: list(range(model.n_items)),
